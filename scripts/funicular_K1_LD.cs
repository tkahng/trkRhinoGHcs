...
#region Members
  ...
  public Line LinePerp(List<Point3d> points, DataTree<int> lineVertex, DataTree<int> vertexVertexes, bool one, bool two, int i, double width, out bool isEnd)
  {
    int i0, i1;
    double signe = 1.0;
    if (one)
  ...
  public class LDSide
  {
    ...
    {
      LDSideSet(side.nodeNumber, side.point, side.direction, side.isEnd);
    }
    public LDSide(int n, Point3d pt, Vector3d dir, bool end)
    {
      nodeNumber = n;
    ...
    public void LDSideSet(int n, Point3d pt, Vector3d dir, bool end)
    {
      nodeNumber = n;
      point = new Point3d(pt);
      direction = new Vector3d(dir);
